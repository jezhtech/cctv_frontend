...
export class AttendanceService {
    ...
    try {
      ...
      if (filters?.cameraId) {
        records = records.filter(
          (record) => record.camera_id === filters.cameraId,
        );
      }
      ...
      if (filters?.dateTo) {
        records = records.filter(
          (record) => new Date(record.created_at) <= new Date(filters.dateTo!),
        );
      }
      ...
      if (filters?.minConfidence) {
        records = records.filter(
          (record) => record.confidence_score >= filters.minConfidence!,
        );
      }

      if (filters?.maxConfidence) {
        records = records.filter(
          (record) => record.confidence_score <= filters.maxConfidence!,
        );
      }
  ...
  async getCameraAttendance(
    cameraId: string,
    filters?: Omit<AttendanceFilters, "cameraId">,
  ): Promise<Attendance[]> {
    try {
  ...
  async getAttendanceSummaryByUser(
    ...
    try {
      const summary = await attendanceApi.getAttendanceSummaryByUser(
        userId,
        dateFrom,
        dateTo,
      );
      ...
      console.error(
        `Failed to get attendance summary for user ${userId}:`,
        error,
      );
      throw error;
    }
  ...
  async calculateAttendanceRate(
    ...
    try {
      const summary = await this.getAttendanceSummaryByUser(
        userId,
        dateFrom,
        dateTo,
      );
      ...
      console.error(
        `Failed to calculate attendance rate for user ${userId}:`,
        error,
      );
      return 0;
    }
